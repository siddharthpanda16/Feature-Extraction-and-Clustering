...
evals,evecs = np.linalg.eigh(W)         # Eigen Value and Vector of Within Class matrix

idx = np.argsort(evals)[::1]            # sort in ascending order as we need to minimize the within class matrix
evals = evals[idx]                      #eigen values
evecs = evecs[:,idx]                    #eigen vectors

r = 2 # since we need to reduce the dimension to n*2

Vec_r = evecs[:,:r] # get first r eigenvectors : r = 2 in our case
